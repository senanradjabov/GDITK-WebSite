...
from src.exceptions import StaffNotFind, DepartmentNotFind
from src.faculty.models import Staff, Department
from src.faculty.schemas import StaffSchema, DepartmentSchema, DepartmentFullResponse
...
class StaffRepository(BaseRepository):
    ...
    async def insert_data(
        ...
        async with async_session_factory() as session:
            if department_id:
                check_department_id: DepartmentSchema | None = await DepartmentRepository.find_one_or_none(
                    id=department_id
                )
...
class DepartmentRepository(BaseRepository):
    ...
    async def insert_data(
        ...
        async with async_session_factory() as session:
            check_staff_id: StaffSchema | None = await StaffRepository.find_one_or_none(id=head_of_department_id)
    ...
    async def get_one_data(cls, slug: str):
        async with async_session_factory() as session:
            smtp = select(
                ...
                selectinload(cls.model.head_of_department)
            ).where(
                cls.model.slug == slug
            )
    ...
    async def get_full_data(cls, slug: str):
        async with async_session_factory() as session:
            smtp = select(
                ...
                selectinload(cls.model.head_of_department),  # Загрузка главы департамента
